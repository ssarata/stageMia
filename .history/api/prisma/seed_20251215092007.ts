...
async function main() {
  ...
  await prisma.role.update({
    ...
    data: {
      permissions: {
        connect: mPermissionsObjects.map(p => ({ id: p!.id }))
      }
    }
  });
  console.log(`✅ m: ${mPermissionsObjects.length} permissions assignées`);
