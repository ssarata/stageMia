...
async function main() {
  ...
  const lecteurRole = createdRoles.find(r => r.name === 'PROVISEUR')!;
  ...
  const lecPermissionsObjects = lecPermissions
  ...
  await prisma.role.update({
    ...
    data: {
      permissions: {
        connect: lecPermissionsObjects.map(p => ({ id: p!.id }))
      }
    }
  });
  console.log(`✅ lec: ${lecPermissionsObjects.length} permissions assignées`);
