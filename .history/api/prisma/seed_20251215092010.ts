...
async function main() {
  ...
  await prisma.role.update({
    ...
    data: {
      permissions: {
        connect: miPermissionsObjects.map(p => ({ id: p!.id }))
      }
    }
  });
  console.log(`✅ mi: ${miPermissionsObjects.length} permissions assignées`);
